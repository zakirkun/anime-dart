...
abstract class GetWatchedEpisodes {
  Future<Either<BrowsingException, List<Episode>>> call();
}

class GetWatchedEpisodesImplementation implements GetWatchedEpisodes {
  final BrowsingRepository repository;

  GetWatchedEpisodesImplementation({
    this.repository,
  });
  ...
  Future<Either<BrowsingException, List<Episode>>> call() async {
    try {
      ...
      return Left(UnableToFetchDataException(
          "An error ocurred, and not was possible to fetch the Watched Episodes"));
    }
  }
}
